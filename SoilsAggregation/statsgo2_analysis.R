#this is essentially SSURGO_analysis_v2.R but with some minor differences because statsgo2 populates data for minor components but SSURGO typically does not
dirnumber <- 2 #argument for manually running function
run_dup_ck <- 'no' #argument for manually running function
library(plyr)
library(dplyr)
library(nnet)
...
#fill in restriciton information for minor components (this is more robust version to handle multiple possible restrictions)
  ssurgo_comp$reskind[which(is.na(ssurgo_comp$reskind))] <- 'None' #statsgo2 minor components have populated data, so removed majcompflag portion of selection
...
#   i <- which(is.na(ssurgo_comp$reskind) & ssurgo_comp$majcompflag=='No') #this means that major components will not have their reskinds adjusted by the most common reskind for a given component name; also any minor component that had an identified reskind will not be adjusted
...
#   for (j in 1:length(i)){
#     reskind_synthesis_row <- match(ssurgo_comp$compname[i[j]], reskind_synthesis$compname)
...
#       ssurgo_comp$reskind[i[j]] <- reskind_synthesis[reskind_pick][reskind_synthesis_row, ]#have to use comma here to reference the row, because column number indexing does not drop the 'data.frame' class when column is returned
#       if (ssurgo_comp$reskind[i[j]] != 'None') {
#         ssurgo_comp$resdept_r[i[j]] <- reskind_synthesis[ ,paste('resdept_reskind', as.character(reskind_pick-1), sep = '')][reskind_synthesis_row] #column indexing is by column names determined by reskind_pick
#       }
#       next
#     }
...
#       ssurgo_comp$reskind[i[j]] <- reskind_synthesis[reskind_pick][reskind_synthesis_row, ]#have to use comma here to reference the row, because column number indexing does not drop the 'data.frame' class when column is returned
#       if (ssurgo_comp$reskind[i[j]] != 'None') {
#         ssurgo_comp$resdept_r[i[j]] <- reskind_synthesis[ ,paste('resdept_reskind', as.character(reskind_pick-1), sep = '')][reskind_synthesis_row]
#       }
#       next
#     }
#     ssurgo_comp$reskind[i[j]] <- reskind_synthesis$reskind1[reskind_synthesis_row]
#     if (ssurgo_comp$reskind[i[j]] != 'None') {
#       ssurgo_comp$resdept_r[i[j]] <- reskind_synthesis$resdept_reskind1[reskind_synthesis_row]
#     }
#   }
...
#export csv file of component names with conficting kinds of restrictions
  # y <- vector()
  # for (m in 1:length(save_compnames)) {
  #   z <- which(ssurgo_comp$compname==save_compnames[m] & ssurgo_comp$majcompflag=='Yes')
  #   y <- c(y, z)
  # }
  # comps_conflicting_reskinds <- ssurgo_comp[y,] 
  # setwd(outpath)
  # write.csv(comps_conflicting_reskinds, paste(watershed_name, '_comps_conflicting_reskinds.csv', sep=''), row.names = FALSE)
...
#correct deep hz names from OSD database
  setwd(OSDdir)
  ...
  txt15 <- paste('There are', length(unique(ssurgo_comp$compname[ssurgo_comp$hzname_deepest_simplified=='H' | is.na(ssurgo_comp$hzname_deepest_simplified)])), 'unique component names without a designated deep horizon with no identified lithic, paralithic, or densic contact.', '\n', 'See _comps_hzname_deepest_isNA.csv for specific component names.  This affects a total of', round(sum(ssurgo_comp$hectares[ssurgo_comp$hzname_deepest_simplified=='H' | is.na(ssurgo_comp$hzname_deepest_simplified)]), digits = 0), 'hectares.')
  ...
  comps_hzname_deepest_isNA <- as.data.frame(unique(ssurgo_comp$compname[ssurgo_comp$hzname_deepest_simplified=='H' | is.na(ssurgo_comp$hzname_deepest_simplified)]))
  ...
  ssurgo_comp$reskind[which(ssurgo_comp$hzname_deepest_simplified=='Cr')] <- 'Paralithic bedrock'
  ssurgo_comp$reskind[which(ssurgo_comp$hzname_deepest_simplified=='R')] <- 'Lithic bedrock'
  ssurgo_comp$reskind[which((ssurgo_comp$reskind=='None' | ssurgo_comp$reskind=='Undefined') & ssurgo_comp$hzname_deepest_simplified=='Cd')] <- 'Densic material'
  ...
  area_weighted_average <- function(df, varname) {
    ...
    for (j in 1:length(i)){
      ...
      if (is.na(compname_reskind)) {
        next(print(paste(rownames(data_by_compname)[look_up_index], 'is missing reskind data to determine which AWC value to paste.')))
      }
...
#now fill in remaining NAs with all_watershed synthesis data
  # i <- which(is.na(ssurgo_comp$awc_H2Ocm))
  # for (j in 1:length(i)){
  #   ssurgo_comp$awc_H2Ocm[i[j]] <- cross_watershed_awc$mean_awc_H2Ocm[match(ssurgo_comp$compname[i[j]], cross_watershed_awc$compname)]
  # }
...
#now fill-in remaning NAs with all_watershed synthesis created in 'master_soil_comp_processing.R'
  # i <- which(is.na(ssurgo_comp$awc_soilthickness_cm))
  # for (j in 1:length(i)){
  #   ssurgo_comp$awc_soilthickness_cm[i[j]] <- cross_watershed_awc$mean_awc_soilthickness_cm[match(ssurgo_comp$compname[i[j]], cross_watershed_awc$compname)]
  # }
...
#percent rock outcrop, still need to account for major soil components with no restriction data but with simplified deep horizon as 'R' from database correction work; perhaps if deep horizon AWC == 0, then assume it's R
  ...
  f <- which(ssurgo_comp$hzname_deepest_simplified=='H' | is.na(ssurgo_comp$hzname_deepest_simplified))
...
#loop through all the statsgo2 data
aggregate_SSURGO(2, 'no') #only run on directory 2, the one for  statsgo2
